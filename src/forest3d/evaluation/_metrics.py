...
def match_instances(target: np.ndarray,
                    prediction: np.ndarray,
                    invalid_tree_id: int = -1,
                    iou_treshold: float = 0.5) -> Tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    r"""
    ...
    for target_id in target_tree_ids:
        ...
        if iou > iou_treshold:
            ...
            metrics.append({'Target': target_id,
...
def instance_segmentation_metrics(matched_target_ids: np.ndarray,
                         matched_predicted_ids: np.ndarray,
                         segmentation_metrics: pd.DataFrame,
                         invalid_tree_id: int = -1) -> Dict[str, float]:
    """
...
def semantic_segmentation_metrics(target: np.ndarray,
                                  prediction: np.ndarray,
                                  class_map: Dict[str, int],
                                  aggregate_classes: Dict[str, List[int]] = None) -> Dict[str, float]:
    """
    ...
    metrics[f'm_iou'] = np.array(ious).mean()
    metrics[f'm_precision'] = np.array(precisions).mean()
    metrics[f'm_recall'] = np.array(recalls).mean()
    ...
    metrics[f'm_iou_aggregated'] = np.array(ious_aggregated).mean()
    metrics[f'm_precision_aggregated'] = np.array(precisions_aggregated).mean()
    metrics[f'm_recall_aggregated'] = np.array(recalls_aggregated).mean()
