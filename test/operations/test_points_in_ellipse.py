...
class TestPointsInEllipse:
    ...
    def test_points_in_diagonal_ellipse(self, storage_format: str, scalar_type: np.dtype):
        ellipse = np.array([1, 1, 4, 2, np.pi / 4], scalar_type, order=storage_format)
        xy = np.array(
            [[1, 1], [0.9 + 4 / np.sqrt(2), 0.9 + 4 / np.sqrt(2)], [2.1, 1.1], [2.1, -1.1], [4.1, 4]],
            dtype=scalar_type,
            order=storage_format,
        )

        in_ellipse = points_in_ellipse(xy, ellipse)
        expected_in_ellipse = np.array([True, True, True, False, False], dtype=scalar_type)
    ...
    @pytest.mark.parametrize("storage_format", ["C", "F"])
    @pytest.mark.parametrize("scalar_type", [np.float32, np.float64])
    def test_points_in_vertical_ellipse(self, storage_format: str, scalar_type: np.dtype):
        ellipse = np.array([1, 1, 4, 2, np.pi / 2], scalar_type, order=storage_format)
        xy = np.array([[1, 1], [1, 4], [4, 1]], dtype=scalar_type, order=storage_format)
